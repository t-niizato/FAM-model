...
Threshold-0.85 supporting version of the individual-flight analysis figure.
The milling and schooling state masks use the 0.85 quantile. The metrics CSV
files supplied through --data-root must also have been generated with the
0.85 threshold settings.
"""
...
def make_threshold(series, q=REGIME_QUANTILE):
...
def reconstruct_individual_flights_from_npz(
    ...
    kind,
    threshold,
    milling_q=REGIME_QUANTILE,
    schooling_q=REGIME_QUANTILE,
):
...
def plot_figure6S_B_sample_fit(data_root: Path, out_dir: Path):
    setup_style()
    ...
    out_pdf = out_dir / "Figure_S6_B.pdf"
    out_png = out_dir / "Figure_S6_B.png"
...
def plot_figure6S_C_alpha_violin(data_root: Path, out_dir: Path):
    setup_style()
    ...
    out_pdf = out_dir / "Figure_S6_C.pdf"
    out_png = out_dir / "Figure_S6_C.png"
...
def main():
    ...
    ap.add_argument(
        "--data-root",
        default="data/processed/figureS6",
        help="Directory containing threshold-0.85 processed data.",
    )
    ap.add_argument(
        "--out",
        default="figures/output",
        help="Output directory for Figure 6S panels.",
    )
    ...
    plot_figure6S_B_sample_fit(data_root, out_dir)
    plot_figure6S_C_alpha_violin(data_root, out_dir)
